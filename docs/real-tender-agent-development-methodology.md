...
## Practical-Agent product and workflow rules

- The audience may be a client, consultant, reviewer, or internal operator. Every practical-Agent Overview must use the audience-neutral ordered contract `outcome promise → input → Agent transformation → finished output → primary action → trust/negative-scope boundary`; the copy and workflow remain Agent-specific.
- The first viewport should make `what the user provides → what the Agent does → what the user receives` understandable in seconds. The finished product, not explanatory documentation or a duplicate product hero, is the visual hero.
- An Overview is not complete because its source contains the right headings. Registry-driven semantic hooks must identify every contract part, and rendered-browser evidence at 1280×720 and 1440×900 must show the outcome, input, transformation, and finished output without initial scrolling. The primary action and trust/negative-scope boundary remain mandatory, visible, and reachable; an Agent-specific contract may require them in the first viewport, as TenderMatch does. The finished output must have at least 1.25 times the rendered area of either the input or transformation stage.
- Standardize this orientation contract only. Do not force client, consultant, or internal products into identical navigation, page sequences, workflows, or domain visuals.
- The primary result must match the Agent’s purpose. TenderBalance foregrounds reviewable FIN-1 and FIN-2 tender forms while the digitized statement remains their traceable evidence layer; TENDER LOGISTICS COST foregrounds one reconciled preliminary logistics estimate and its commercial consequence.
...
| TenderBalance’s Overview presented the digitized balance sheet as the finished product even though the client’s intended outcome is a tender-ready FIN package. | A canonical intermediate evidence artifact was mistaken for the client-facing deliverable. | Reframed digitization, reconciliation, and mapping as the Agent transformation and foregrounded reviewable FIN-1 and FIN-2 forms as the finished output. | The Overview must visualize the actual client deliverable, not the internal evidence layer used to produce it. | `tests/tender-balance-client-workflow.test.mjs` FIN-package Overview assertions and rendered `/balance-sheet-review` evidence. |
| TenderMatch contained the correct input → Agent work → output story in source, but the rendered first desktop screen showed a separate oversized product hero and Case strip while the story began below the fold. | Source-string presence and per-Agent assertions were mistaken for experiential completion; no universal rendered first-viewport gate protected the hierarchy. | Consolidated TenderMatch identity and authority into its Overview, moved Case controls below the product story, introduced audience-neutral semantic Overview hooks, and added registry plus rendered-browser gates. | A practical-Agent Overview is complete only when the served route shows the ordered product story, dominant finished output, primary action, and truthful authority boundary at representative viewports. | `tests/practical-agent-overview-contract.test.mjs`; TenderMatch rendered measurements at 1280×720 and 1440×900; focused TenderMatch Overview assertions. |
...
The policy manifest makes the lifecycle order, knowledge layers, provenance classes, trust dimensions, workflow states, practical-Agent Overview contract, failure layers, artifact-parity fields, blocking rule, failure-audit fields, and minimum test families machine-checkable. `tests/real-tender-agent-development-methodology.test.mjs` protects that contract and verifies that project instructions and this document remain discoverable. `tests/practical-agent-overview-contract.test.mjs` is registry-driven: every registered real practical Agent must declare an audience, implementation/composition source, ordered semantic hooks, representative desktop viewports, output-dominance threshold, and visible trust boundary.
...
5. The practical-Agent Overview now has shared semantic and rendered-browser gates. Broader multi-browser, stale-release, and persisted-Case end-to-end coverage remains future test infrastructure.
...
| Practical-Agent Overview | Registry contract, semantic order, rendered first-viewport placement, finished-output dominance, primary action, and trust/negative scope are verified for the intended audience. |
