...
The release script checks the existing protected profile, publishes only the named gateway using the supported Cloud Functions command, verifies provider runtime bounds/new source generation/source label and direct denial, then publishes the matching payload-free Hosting and verifies both domains by exact public hashes plus protected-path denial. Nonzero deploy exits stop before Hosting. The Cloud Functions command avoids Firebase CLI15.27's unconditional request for actAs on the default App Engine identity, which has broad Editor access. No such default-account grant, invoker change, image cleanup policy or forced deletion is introduced. A failed confidentiality check requires the ordered rollback below; an authenticated owner replay remains a separate release verification.
