...
## Local retrieval/ranking checkpoint — 2026-09-05

The subsequent, not-yet-deployed [retrieval/ranking implementation](tendermatch-retrieval-ranking.md) preserves this source inventory and Formula v1.1 while replacing universal browser pair narratives with normalized inputs, compact versioned scores, bounded retrieval and selected-pair explanations. It includes a separately tested PostgreSQL/pgvector result-store adapter; the active local preview is still an in-memory pinned-data service and full TORS remains disabled. See its [completion evidence](evidence/tendermatch-retrieval-validation.md) for exact validation, benchmark limitations and preview identities. This local checkpoint does not supersede the deployed-release identity or authorize source replacement, live database writes, authentication rollout, outreach or deployment.
