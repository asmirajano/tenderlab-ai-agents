...
test("binds the deployed supplier snapshot to manifest hashes and exact v1.3 counts", async () => {
  ...
  assert.deepEqual(manifest.counts, { tenders: 60, suppliers: 17, evidence: 289, evaluations: 1020, numericEvaluations: 0, missingEvaluations: 1020 });
  ...
  assert.equal(runtime.evaluationSummary.numeric, 0);
  assert.equal(runtime.evaluationSummary.missing, 1020);
  assert.ok(runtime.evaluations.every((entry) => entry.value === null && entry.valueClass === "MISSING"));
