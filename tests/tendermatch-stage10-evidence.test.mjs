...
test('final ledger binds retained sealed counts and explicitly pending owner/runtime gates without refreshing Neon',{skip:!enabled},async()=>{
  const l=await read('docs/evidence/tendermatch-all-to-all-ledger.json');await bind(l.sourceBinding);assert.equal(Object.keys(l.sourceBinding).length,12);assert.equal(l.population.universe,2027961);assert.equal(l.population.formulaScored,707660);assert.equal(l.population.formulaUnscored,1320301);assert.equal(l.population.shortlist,28034);assert.equal(l.population.review,18531);assert.equal(l.population.audit,9503);assert.equal(l.population.positiveRetrieval,24007);assert.equal(l.population.stage7PlannedIntents,500);assert.equal(l.stage7.ownerInstallation,'PENDING_NOT_CONFIRMED');assert.equal(l.stage7.persistence,'PENDING_NOT_EXECUTED');assert.equal(l.population.actualModelCalls+l.population.inputTokens+l.population.outputTokens+l.population.cost,0);assert.equal(l.state,'LOCAL_IMPLEMENTATION_VALIDATED_CLOUD_INCOMPLETE');assert.equal(l.sealed.reduce((n,s)=>n+s.tableBytes,0),2619154432);
