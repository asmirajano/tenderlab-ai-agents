...
test('final ledger binds retained sealed counts and the completed isolated Stage 7 without refreshing sources',{skip:!enabled},async()=>{
  ...
  // The subsequent authorized Stage 8 host adds one separate Fetch contract; it
  // does not change any sealed algorithm, prior frontend or installed migration.
  // Separately authorized Stage 9 changes only its pre-existing isolated UI/transport
  // and a non-secret audience contract. The production UI and algorithms stay pinned.
  const protectedDiff=execFileSync('git',['diff',l.baseCommit,'--','apps','packages','db','migrations',':(exclude)packages/tendermatch/src/hosted-stage8.ts',':(exclude)packages/tendermatch/src/stage9-browser-contract.ts',':(exclude)apps/tender-apps/src/tendermatch-all-to-all-api.ts',':(exclude)apps/tender-apps/src/tendermatch-all-to-all.tsx',':(exclude)apps/tender-apps/src/tendermatch-all-to-all.css',':(exclude)db/tendermatch-dev/110-stage8-reader-up.sql',':(exclude)db/tendermatch-dev/110-stage8-reader-down.sql'],{cwd:root,encoding:'utf8'});assert.equal(protectedDiff,'','Stage 10 must not mutate prior frontend, algorithms or installed schema');
});
