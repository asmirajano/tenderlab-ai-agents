...
test('final ledger binds retained sealed counts and the completed isolated Stage 7 without refreshing sources',{skip:!enabled},async()=>{
  ...
  const protectedDiff=execFileSync('git',['diff',l.baseCommit,'--','apps','packages','db','migrations',':(exclude)packages/tendermatch/src/hosted-stage8.ts',':(exclude)db/tendermatch-dev/110-stage8-reader-up.sql',':(exclude)db/tendermatch-dev/110-stage8-reader-down.sql'],{cwd:root,encoding:'utf8'});assert.equal(protectedDiff,'','Stage 10 must not mutate prior frontend, algorithms or installed schema');
});
