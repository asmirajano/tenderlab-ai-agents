...
test("ends the Overview after the approved infographic and authority boundary", async () => {
  const [page, styles, roleStyles] = await Promise.all([
    read("apps/tender-apps/src/tendermatch-app.tsx"),
    read("apps/tender-apps/src/tendermatch.css"),
  ...
  assert.match(orientation, /AgentRoleCallout[\s\S]+className="tb3-role-callout"[\s\S]+imageAlt="TenderLab analyst reviewing tender and supplier evidence"[\s\S]+imageSrc="\/tendermatch\/illustrations\/tendermatch-consultant\.png"/);
  ...
  assert.match(roleStyles, /\.agent-role-callout__portrait \{[^}]*border-radius: 42% 58% 48% 52% \/ 35% 42% 58% 65%/);
  assert.match(roleStyles, /\.agent-role-callout__tags \{[^}]*flex-wrap: wrap/);
