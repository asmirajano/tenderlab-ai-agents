...
- require every real practical-Agent Overview—whether for a client, consultant, reviewer, or internal operator—to implement the shared ordered contract `outcome promise → input → Agent transformation → finished output → primary action → trust/negative-scope boundary`; preserve Agent-specific navigation and workflows;
- do not mark an Overview complete from source copy alone: run the registry-driven semantic contract test and retain rendered-browser evidence at the policy viewports proving first-screen placement, finished-output dominance, responsive order, accessible actions, and visible authority boundaries.
