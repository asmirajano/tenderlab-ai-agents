...
export const TENDERMATCH_EXPLORATORY_ENGINE_VERSION = "tendermatch-match-formula/1.0.0" as const;
export const TENDERMATCH_EXPLORATORY_POLICY_VERSION = "tendermatch-evidence-aware-goods-works/1.0.0" as const;
...
export type ExploratoryMatchEvaluation = {
  ...
  value: number | null; valueClass: "ESTIMATED" | "MISSING"; label: "Preliminary notice-level match"; pairStatus: TenderMatchPairStatus; dataCoverage: number; evidenceConfidence: number;
...
export function evaluateExploratoryPair(tender: TenderRecord, supplier: SupplierProfileApiRecord, evidence: SupplierEvidenceApiRecord[], evaluatedAt: string): ExploratoryMatchEvaluation {
  ...
  const gates: TenderMatchMandatoryGate[] = [
    gate("PROCUREMENT_TYPE_SUPPLIER_ROLE", "Procurement type / supplier role", roleState, !inScope ? "Current Formula v1.0 is limited to GOODS and WORKS notices." : compatible ? `The ${supplier.classification} supplier role matches the ${tender.procurementType} notice.` : `The ${supplier.classification} supplier role does not match the ${tender.procurementType} notice.`),
  ...
  const value = inScope && compatible && dataCoverage >= 50 && assessedWeight > 0 ? Math.round(100 * assessed.reduce((sum, entry) => sum + (entry.fitLevel ?? 0) * entry.weight, 0) / (5 * assessedWeight)) : null;
  ...
  const failedGates = gates.filter((entry) => entry.state === "FAIL"); const unknownGates = gates.filter((entry) => entry.state === "UNKNOWN");
  const pairStatus: TenderMatchPairStatus = !inScope ? "UNASSESSED" : failedGates.length ? "BLOCKED_INELIGIBLE" : dataCoverage < 50 ? "UNASSESSED" : value === null ? "UNASSESSED" : value < 60 ? "NO_MATCH" : value >= 85 && dataCoverage >= 85 && evidenceConfidence >= 75 && !unknownGates.length ? "BINGO_MATCH" : value >= 75 && dataCoverage >= 70 && evidenceConfidence >= 60 ? "STRONG_CANDIDATE" : unknownGates.length || evidenceConfidence < 60 ? "NEEDS_VERIFICATION" : "POTENTIAL_MATCH";
  const mainReason = !inScope ? "CURRENT_SCOPE_GOODS_WORKS_ONLY" : failedGates.length ? failedGates[0].code : dataCoverage < 50 ? "MINIMUM_DATA_COVERAGE_NOT_MET" : pairStatus === "NO_MATCH" ? "SUPPORTED_FIT_BELOW_THRESHOLD" : pairStatus === "NEEDS_VERIFICATION" ? "PROMISING_FIT_REQUIRES_VERIFICATION" : "PRELIMINARY_NOTICE_LEVEL_FIT";
  ...
  return {
    ...
    engineVersion: TENDERMATCH_EXPLORATORY_ENGINE_VERSION, policyVersion: TENDERMATCH_EXPLORATORY_POLICY_VERSION, evaluatedAt, value, valueClass: value === null ? "MISSING" : "ESTIMATED", label: "Preliminary notice-level match", pairStatus, dataCoverage, evidenceConfidence,
    ...
    limitations: ["Preliminary notice-level match only; it is not a formal bid evaluation, legal eligibility verdict, winner prediction, or Bid/No-Bid decision.", "The supplier batch contains zero VERIFIED claims. STATED_UNVERIFIED and INFERRED classes remain explicit.", "Unknown criteria are excluded from the score denominator and reduce Data Coverage; MISSING is never converted to zero.", "Supplier readiness, deadline urgency and consultant disposition are separate and never add Match Score points."], consultantDecision: "pending",
  };
}
...
export function assessmentFromExploratoryEvaluation(evaluation: ExploratoryMatchEvaluation): MatchAssessment {
  ...
  return {
    ...
    auditedMatch: { engineVersion: evaluation.engineVersion, policyVersion: evaluation.policyVersion, value: evaluation.value, valueClass: evaluation.valueClass, label: evaluation.value === null ? "insufficient-evidence" : evaluation.value >= 75 ? "strong" : evaluation.value >= 60 ? "review" : "weak", components: evaluation.criteria.map((entry) => ({ ...entry, value: entry.fitLevel === null ? null : entry.fitLevel * 20 })), evidenceIds: evaluation.evidenceIds, reasonCodes: evaluation.reasonCodes, missingInputs: evaluation.missingInputs, legacyScore: null, legacyDelta: null, method: "Formula v1.0: weighted fit over assessed criteria only; Data Coverage preserves all applicable missing criteria. Mandatory gates remain separate.", status: evaluation.pairStatus, dataCoverage: evaluation.dataCoverage, evidenceConfidence: evaluation.evidenceConfidence, gates: evaluation.mandatoryGates, mainReason: evaluation.mainReason, blockers: evaluation.blockers, noticeLabel: evaluation.label },
