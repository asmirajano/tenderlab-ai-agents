...
import {STAGE9_BROWSER} from './stage9-browser-contract.ts';

export const HOSTED_VERSION='tendermatch-neon-readonly/1.0.0';
export type HostedSession={tokenHash:string;subject:string;tenantId:string;issuedAt:number;expiresAt:number;browserAudience?:string;browserOrigin?:string};
...
export function hostedSessions(seeds:HostedSession[],clock=Date.now){
  ...
  for(const s of seeds){
    ...
    if((s.browserAudience!==undefined||s.browserOrigin!==undefined)&&(s.browserAudience!==STAGE9_BROWSER.audience||s.browserOrigin!==STAGE9_BROWSER.origin||s.expiresAt-s.issuedAt>STAGE9_BROWSER.sessionMs))throw new Error('Invalid browser audience');
    entries.set(s.tokenHash,{seed:{...s},window:-1,count:0});
  }
  return {authenticate(value:string|null,origin:string|null=null,audience:string|null=null):Principal{
    ...
    if(entry.seed.browserAudience ? origin!==entry.seed.browserOrigin||audience!==entry.seed.browserAudience : origin!==null||audience!==null)return fail(403,'SESSION_AUDIENCE_REQUIRED');
...
export function createHostedFetch({store,sessions,allowedOrigins,codeHash,clock=Date.now}:{store:ReadStore;sessions:ReturnType<typeof hostedSessions>;allowedOrigins:string[];codeHash:string;clock?:()=>number}){
  ...
  return async(request:Request):Promise<Response>=>{
    ...
    const send=(status:number,payload:unknown)=>{
      ...
      if(origin&&origins.has(origin))headers['Access-Control-Expose-Headers']='X-TenderMatch-Code, X-TenderMatch-Runtime, Server-Timing';
      if(status===204){headers['Access-Control-Allow-Methods']='GET';headers['Access-Control-Allow-Headers']='Authorization, X-TenderMatch-Audience';headers['Access-Control-Max-Age']='60';headers.Vary+=', Access-Control-Request-Method, Access-Control-Request-Headers';}
    ...
    try{
      ...
      // Preflight conveys no data/authority and never acquires a DB connection.
      if(request.method==='OPTIONS'&&origin){
        const headers=(request.headers.get('access-control-request-headers')??'').toLowerCase().split(',').map(h=>h.trim());
        if(origin!==STAGE9_BROWSER.origin||request.headers.get('access-control-request-method')!=='GET'||headers.length!==2||!headers.includes('authorization')||!headers.includes('x-tendermatch-audience')||!new RegExp('^'+API_PREFIX+'/(health|(?:suppliers|tenders)/[a-f0-9-]+/results|pairs/[a-f0-9-]+/[a-f0-9-]+)$').test(url.pathname))fail(403,'READ_ONLY_PREFLIGHT_REQUIRED');
        queryBinding(url,['binding','limit','cursor']);
        return send(204,{});
      }
      const principal=sessions.authenticate(request.headers.get('authorization'),origin,request.headers.get('x-tendermatch-audience'));
