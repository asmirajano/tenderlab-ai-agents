...
const COLUMN_WIDTHS = [
  38, 24, 18, 36, 24, 54, 34, 18, 24, 36, 24, 14, 18,
...
export async function tenderMatchFormulaToExcel(
  ...
  workbook.subject = "TenderMatch Match Formula v1.0 pair-evaluation audit";
  workbook.title = "TenderMatch Formula v1.0 Audit";

  const overview = workbook.addWorksheet("Formula Summary", { views: [{ state: "frozen", ySplit: 1 }] });
  overview.columns = [{ width: 34 }, { width: 64 }];
  overview.addRow(["TenderMatch Match Formula v1.0", "Audit summary"]);
  overview.addRows([
    ...
    ["Numeric preliminary scores", summary.numeric],
    ["MISSING scores", summary.missing],
    [],
    ["Pair status", "Count"],
  ...
  for (const values of rows) {
    const row = pairs.addRow(values.map((value, index) => index === 24 && typeof value === "string" ? new Date(value) : value));
  ...
  pairs.getColumn(14).numFmt = '0"%"';
  pairs.getColumn(15).numFmt = '0"%"';
  pairs.getColumn(25).numFmt = "yyyy-mm-dd hh:mm:ss";
